...
public enum PrototypeBossAnimationState
{
    Idle,
    Telegraph,
    Dash,
...
{
    [SerializeField] private PrototypeBossConfig _config = new PrototypeBossConfig();
    ...
    private readonly HashSet<PrototypeHealth> _damagedTargets = new HashSet<PrototypeHealth>();
    ...
    private PrototypeBossAnimationState _animationState = PrototypeBossAnimationState.Idle;
    ...
    {
        CacheComponents();
        _config = PrototypeBossConfigLoader.Sanitize(_config);
        EnsureHitboxVisuals();
    }
    ...
    public void Initialize(PrototypeBossConfig config, Transform target)
    {
        CacheComponents();

        _config = PrototypeBossConfigLoader.DeepClone(config);
        _target = target;
        _initialized = true;
    ...
    public void Initialize(Transform target)
    {
        ...
        _config = PrototypeBossConfigLoader.DeepClone(_config);
        _target = target;
        _initialized = true;
        RefreshRuntimeConfig(resetBossState: true, preserveHealthRatio: false);
    }

    public void SetSerializedConfig(PrototypeBossConfig config)
    {
        _config = PrototypeBossConfigLoader.DeepClone(config);
    }
    ...
    public void RefreshRuntimeConfig(bool resetBossState = false, bool preserveHealthRatio = true)
    {
        ...
        {
            _config = PrototypeBossConfigLoader.CreateDefault();
        }
        ...
        if (resetBossState)
        {
            ...
            SetAnimationState(PrototypeBossAnimationState.Idle);
        }

        if (_health != null)
        {
            _health.Configure(
                PrototypeFaction.Enemy,
                _config.core.maxHealth,
                _config.core.normalColor.ToColor(),
                _config.core.deadColor.ToColor(),
                preserveHealthRatio);
            _health.SetRespawnEnabled(true);
            _health.ConfigureRespawn(transform.position, 1.25f, new MonoBehaviour[] { this });
        }
    ...
    public PrototypeBossConfig CreateConfigSnapshot()
    {
        return PrototypeBossConfigLoader.DeepClone(_config);
    }
    ...
    private void MoveWithinArena(float deltaTime)
    {
        ...
        {
            moveDirection = Mathf.Sign(_target.position.x - transform.position.x);
        }
    ...
    {
        if (_target == null || _currentPhase == null || _currentPhase.patterns == null || _currentPhase.patterns.Length == 0)
        {
            return;
        }
    ...
    private void BeginPattern(BossPatternConfig pattern)
    {
        ...
        SetAnimationState(PrototypeBossAnimationState.Telegraph);
    ...
    private void EnterExecute()
    {
        ...
        if (_activePatternType == BossPatternType.LeapSlam)
        {
            SetAnimationState(PrototypeBossAnimationState.Leap);
            _leapStartPosition = transform.position;
        }
        else if (_activePatternType == BossPatternType.ProjectileFan)
        {
            SetAnimationState(PrototypeBossAnimationState.Shoot);
        ...
        {
            SetAnimationState(PrototypeBossAnimationState.Dash);
            _dashDirection = _target != null ? Mathf.Sign(_target.position.x - transform.position.x) : _facing;
    ...
    private void ApplyDashDamage(float dashDirection)
    {
        ...
        for (int i = 0; i < hits.Length; i++)
        {
            ...
            if (target == null || target == _health || _damagedTargets.Contains(target))
            {
                continue;
            }
    ...
    private void FireProjectileVolley()
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            renderer.sprite = PrototypeRuntimeSpriteUtility.WhiteSprite;

            CircleCollider2D circleCollider = projectileObject.AddComponent<CircleCollider2D>();
            circleCollider.isTrigger = true;

            PrototypeBossProjectile projectile = projectileObject.AddComponent<PrototypeBossProjectile>();
    ...
    {
        if (_target == null || _config.core.contactDamage <= 0f)
        {
            return;
        }
    ...
    {
        int newPhaseIndex = 0;
        float healthRatio = _health.HealthNormalized;
    ...
    private static float[][] BuildCooldownCache(PrototypeBossConfig config)
    {
    ...
    private void SetAnimationState(PrototypeBossAnimationState newState)
    {
    ...
    {
        _leapStartPosition = transform.position;
        float targetX = _target != null ? _target.position.x : transform.position.x;
    ...
    private void EnsureHitboxVisuals()
    {
        ...
        {
            _dashHitboxVisual = CreateHitboxVisual("BossDashHitboxVisual", PrototypeRuntimeSpriteUtility.WhiteSprite, 14);
            _dashHitboxRenderer = _dashHitboxVisual.GetComponent<SpriteRenderer>();
        }

        if (_leapHitboxVisual == null)
        {
            _leapHitboxVisual = CreateHitboxVisual("BossLeapHitboxVisual", PrototypeRuntimeSpriteUtility.CircleSprite, 13);
