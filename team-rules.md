...
├── Fonts/
├── Feel/                  # 외부 공용 에셋 (현재 프로젝트에서 사용하는 경로)
├── Plugins/               # 외부 플러그인 / 셰이더 노드 / DOTween
├── Resources/             # 외부 에셋 설정 파일 (예: DOTweenSettings)
└── ToonFX/                # 외부 VFX 에셋
```
...
- 현재 프로젝트의 외부 에셋은 `Assets/Feel`, `Assets/Plugins`, `Assets/ToonFX`, `Assets/Resources`에 들어간다.
- 외부 에셋 원본 파일은 절대 직접 수정하지 않는다. 필요하면 복사해서 사용한다.
- 이미 임포트된 외부 에셋 폴더는 함부로 `ThirdParty/` 같은 다른 경로로 옮기지 않는다.
  - 에셋마다 경로를 가정하거나 `Resources` 로딩을 사용하는 경우가 있다.
  - 특히 `Assets/Resources/DOTweenSettings.asset` 같은 파일은 `Resources` 규칙을 유지해야 한다.
- 앞으로 새로운 외부 에셋 경로를 통일하고 싶다면, 팀 합의 후 새 에셋부터 규칙을 적용한다.

### 외부 에셋 관련 주의사항

- 외부 에셋은 가능하면 **Unity Package Manager / Asset Store 기본 경로 그대로** 유지한다.
